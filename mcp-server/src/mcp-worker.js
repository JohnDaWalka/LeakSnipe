...
const LANDING_HTML = `<meta charset="UTF-8">
...
/*
  Palette is drawn from the optic itself, and each hue carries one job:
    phosphor green  = the instrument -- live state, reticle, active, scanning
    reticle red     = the target -- leaks and flags ONLY, nothing decorative
    cerulean        = data -- measurements, stat numbers, your own seat
  (cerulean is the flare off coated glass; green and red are the reticle
  illumination modes every real scope ships with.)
  Components read only the semantic tokens below, never the raw ramps, so
...
.scope .tick.tl { top: -1px; left: -1px; border-right: none; border-bottom: none; }
.scope .tick.tr { top: -1px; right: -1px; border-left: none; border-bottom: none; }
.scope .tick.bl { bottom: -1px; left: -1px; border-right: none; border-top: none; }
.scope .tick.br { bottom: -1px; right: -1px; border-left: none; border-top: none; }
...
.brand-mark .ring { stroke: var(--text); }
.brand-mark .cross { stroke: var(--alert); filter: drop-shadow(var(--glow-alert)); }
.brand-mark .tick { fill: var(--alert); }
.brand-mark .flare-core { fill: var(--data); }
.brand-mark .flare-halo { fill: var(--data); opacity: 0.28; }
...
@media (max-width: 640px) {
  .tabs { gap: 1rem; font-size: 0.68rem; overflow-x: auto; -ms-overflow-style: none; scrollbar-width: none; }
...
@media (prefers-reduced-motion: reduce) {
  .hud-mock .live .pip { animation: none; }
  .sweep { display: none; }
}
@keyframes pulse {
  0%, 100% { opacity: 1; }
...
.table-oval {
  position: relative;
  background: radial-gradient(ellipse at center, var(--bg-raised-2) 0%, var(--bg-raised) 72%);
  border: 1px solid var(--line);
  border-radius: 50% / 38%;
  aspect-ratio: 16 / 10;
  overflow: hidden;
}
/* scope crosshair, sitting behind the data as instrument texture */
.table-oval::before,
.table-oval::after {
  content: "";
  position: absolute;
...
.table-oval::before { left: 7%; right: 7%; top: 50%; height: 1px; }
.table-oval::after { top: 7%; bottom: 7%; left: 50%; width: 1px; }
.sweep {
  position: absolute;
  inset: 0;
  background: linear-gradient(180deg, transparent 0%, var(--accent-soft) 48%, transparent 100%);
  height: 40%;
...
.seat .name { color: var(--text-faint); text-transform: uppercase; letter-spacing: 0.03em; font-size: 0.58rem; }
.seat .stat { color: var(--text-dim); }
.seat.flag { border-color: var(--alert); background: var(--alert-soft); box-shadow: var(--glow-alert); }
.seat.flag .stat { color: var(--alert); font-weight: 500; }
.seat.hero { border-color: var(--data); background: var(--data-soft); }
.seat.hero .stat { color: var(--data); }
.seat.s1 { top: 6%; left: 50%; transform: translateX(-50%); }
.seat.s2 { top: 22%; right: 4%; }
.seat.s3 { bottom: 22%; right: 8%; }
.seat.s4 { bottom: 6%; left: 50%; transform: translateX(-50%); }
.seat.s5 { bottom: 22%; left: 8%; }
.seat.s6 { top: 22%; left: 4%; }
.hud-caption {
  margin-top: 0.9rem;
  font-family: var(--font-mono);
  font-size: 0.68rem;
...
/* ---------- section shell ---------- */
section { padding: clamp(3.5rem, 7vw, 5.5rem) 0; border-top: 1px solid var(--line); }
.section-head {
  display: flex;
  justify-content: space-between;
...
.arch-card .k { font-family: var(--font-mono); font-size: 0.68rem; color: var(--data); letter-spacing: 0.08em; text-transform: uppercase; }
.arch-card h3 { font-family: var(--font-body); font-weight: 600; text-transform: none; letter-spacing: 0; font-size: 1.08rem; margin: 0.5rem 0 0.6rem; }
.arch-card p { color: var(--text-dim); font-size: 0.92rem; }
...
.detail-layout.with-rail { grid-template-columns: 1fr 17.5rem; }
@media (max-width: 860px) {
  .detail-layout.with-rail { grid-template-columns: 1fr; }
}
...
.gloss-entry:first-child { border-top: none; }
@media (max-width: 640px) {
  .gloss-entry { grid-template-columns: 1fr; gap: 0.4rem; }
}
.gloss-entry .term {
  font-family: var(--font-mono);
  font-size: 0.95rem;
  ...
  <div class="wrap">
    <button class="brand" data-nav="overview" aria-label="LeakSnipe — back to overview">
      <svg class="brand-mark" viewBox="0 0 32 32" width="21" height="21" aria-hidden="true">
        ...
        <line class="cross" x1="16" y1="20.5" x2="16" y2="28.5" stroke-width="1.6"></line>
        <line class="cross" x1="3.5" y1="16" x2="11.5" y2="16" stroke-width="1.6"></line>
        <line class="cross" x1="20.5" y1="16" x2="28.5" y2="16" stroke-width="1.6"></line>
        <circle class="tick" cx="16" cy="9" r="0.9"></circle>
        <circle class="tick" cx="16" cy="23" r="0.9"></circle>
        <circle class="tick" cx="9" cy="16" r="0.9"></circle>
        <circle class="tick" cx="23" cy="16" r="0.9"></circle>
        <circle class="flare-halo" cx="11.5" cy="11.5" r="3.6"></circle>
        <circle class="flare-core" cx="11.5" cy="11.5" r="1.9"></circle>
      </svg>
      LEAKSNIPE
    </button>
    <div class="tabs" role="tablist" aria-label="LeakSnipe topics">
      <button class="tab-btn" role="tab" id="tab-loop" aria-controls="view-loop" aria-selected="false" data-nav="loop">The Loop</button>
      <button class="tab-btn" role="tab" id="tab-intelligence" aria-controls="view-intelligence" aria-selected="false" data-nav="intelligence">Intelligence</button>
      <button class="tab-btn" role="tab" id="tab-architecture" aria-controls="view-architecture" aria-selected="false" data-nav="architecture">Architecture</button>
      <button class="tab-btn" role="tab" id="tab-leaks" aria-controls="view-leaks" aria-selected="false" data-nav="leaks">Leak Detection</button>
    </div>
  </div>
</div>
  ...
  <header class="hero">
    <div class="wrap hero-grid">
      <div>
        ...
        <div class="cta-row">
          <button class="btn" data-nav="loop">See how it works →</button>
          <button class="btn ghost" data-nav="architecture">Read the architecture</button>
        </div>
      </div>
      <div class="hud-mock scope">
        ...
        <div class="hud-caption">
          ...
          <span>Position-scoped VPIP / PFR</span>
        </div>
      </div>
    </div>
  </header>
    ...
    <div class="wrap">
      ...
      <div class="loop-grid">
        <div class="loop-step">
          ...
          <h3>Track</h3>
          <p>Hands import the moment they're written. BetACR/ACR watch folders are auto-discovered; CoinPoker, GGPoker, PokerStars, 888poker, Ignition and ReplayPoker are supported alongside.</p>
        </div>
        <div class="loop-step">
        ...
        <div class="loop-step">
          ...
          <h3>Review</h3>
          <p>Street-by-street replay with an opponent hole-card visibility toggle. Double-click any hand into its full action log.</p>
        </div>
        <div class="loop-step">
          <span class="num">04</span>
          <h3>Study</h3>
          <p>Range Studio, Monte Carlo equity, CFR+ solves on toy subgames, and stack-depth charts from 5BB to 100BB.</p>
        </div>
        <div class="loop-step">
          <span class="num">05</span>
          <h3>Improve</h3>
          <p>Leak alerts and positional breakdowns turn "I think I'm too loose from the blinds" into an actual number.</p>
        </div>
      </div>
    </div>
  </section>
        ...
        <div>
          ...
          <div class="feature-list">
            ...
            <div class="feature-item">
              ...
              <div><h3>Computes, doesn't estimate</h3><p>Equity claims run through the actual Monte Carlo engine. No percentage reaches you without being calculated first.</p></div>
            </div>
            <div class="feature-item">
    ...
    <div class="wrap">
      ...
      <div class="arch-grid">
        <div class="arch-card">
          ...
          <h3>Local-first SQLite</h3>
          <p>Versioned migrations, a Tauri desktop shell, and a Windows overlay HUD that anchors to the table while you play. Nothing is required to leave your machine.</p>
        </div>
        <div class="arch-card">
      ...
      <div class="stat-strip">
        <div class="stat-cell"><div class="num mono-num">10,000+</div><div class="label">Hands indexed and searchable</div></div>
        <div class="stat-cell"><div class="num mono-num">7</div><div class="label">Poker sites supported</div></div>
        <div class="stat-cell"><div class="num mono-num">6</div><div class="label">Core leak stats, by position</div></div>
      </div>
    </div>
  </section>
    ...
    <div class="wrap">
      ...
      <div class="leak-wrap">
        <table class="leak-table">
          <thead>
            <tr><th>Position</th><th>Hands</th><th>VPIP</th><th>PFR</th><th>AF</th><th>Fold to C-bet</th><th>WTSD</th><th>Flag</th></tr>
          </thead>
          <tbody>
            <tr><td class="pos">UTG</td><td class="num">184</td><td class="num">14.1%</td><td class="num">11.9%</td><td class="num">1.4</td><td class="num">51%</td><td class="num">24%</td><td></td></tr>
            <tr><td class="pos">CO</td><td class="num">311</td><td class="num">27.3%</td><td class="num">19.8%</td><td class="num">1.6</td><td class="num">44%</td><td class="num">29%</td><td></td></tr>
            <tr><td class="pos">BTN</td><td class="num">402</td><td class="num">39.4%</td><td class="num">28.1%</td><td class="num">1.7</td><td class="num">41%</td><td class="num">33%</td><td></td></tr>
            <tr><td class="pos">SB</td><td class="num">298</td><td class="num">21.7%</td><td class="num">15.0%</td><td class="num">1.2</td><td class="num">53%</td><td class="num">22%</td><td></td></tr>
            <tr><td class="pos flagged">BB</td><td class="num flagged">356</td><td class="num flagged">18.5%</td><td class="num flagged">8.3%</td><td class="num flagged">1.1</td><td class="num flagged">57%</td><td class="num flagged">18%</td><td><span class="pill">Leak</span></td></tr>
          </tbody>
        </table>
      </div>
    </div>
...
<section class="view detail-panel" id="view-architecture" role="tabpanel" aria-labelledby="tab-architecture" tabindex="-1" hidden>
  <div class="wrap">
    ...
    <div class="detail-layout with-rail">
      ...
      <aside class="spec-rail scope">
        <span class="tick tl"></span><span class="tick tr"></span><span class="tick bl"></span><span class="tick br"></span>
        <div class="rail-title">Spec sheet</div>
        <div class="spec-row"><span class="k">Desktop</span><span class="v">Tauri + React</span></div>
        <div class="spec-row"><span class="k">Engine</span><span class="v">Python (FastAPI sidecar)</span></div>
        <div class="spec-row"><span class="k">Storage</span><span class="v">SQLite, versioned</span></div>
        <div class="spec-row"><span class="k">Live HUD</span><span class="v">Win32 overlay</span></div>
        <div class="spec-row"><span class="k">Sync target</span><span class="v">Object store + SQL mirror</span></div>
        <div class="spec-row"><span class="k">Protocol</span><span class="v">MCP</span></div>
        <div class="spec-row"><span class="k">DB access</span><span class="v">Read-only, token-gated</span></div>
        <div class="spec-row"><span class="k">Equity engine</span><span class="v">Monte Carlo</span></div>
        <div class="spec-row"><span class="k">Solver</span><span class="v">CFR+ / neural value net</span></div>
      </aside>
    </div>
  </div>
</section>
    ...
    <div class="detail-layout">
      <div>
        <div class="glossary">
          ...
          <div class="gloss-entry">
            <div class="term">Fold to C-bet</div>
            <div class="def">
              <p>How often you fold to a continuation bet after calling or checking to the raiser.</p>
              <p class="why">Past the mid-40s%, you're profitable to bluff on any two cards.</p>
            </div>
          </div>
          <div class="gloss-entry">
          ...
          <div class="gloss-entry">
            <div class="term">3-Bet</div>
            <div class="def">
              <p>How often you re-raise a raise before the flop.</p>
              <p class="why">Too low and opponents open-raise you profitably all day. Too high and you're one all-in away from finding out your range doesn't hold up.</p>
            </div>
          </div>
        </div>
        ...
        <div class="leak-wrap" style="margin-top:1.5rem;">
          <table class="leak-table">
            <thead>
              <tr><th>Position</th><th>Hands</th><th>VPIP</th><th>PFR</th><th>AF</th><th>Fold to C-bet</th><th>WTSD</th><th>Flag</th></tr>
            </thead>
...
<script>
(function () {
  var navTargets = ['overview', 'loop', 'intelligence', 'architecture', 'leaks'];
  var tabs = Array.prototype.slice.call(document.querySelectorAll('.tab-btn'));

  function activate(name, opts) {
    opts = opts || {};
    if (navTargets.indexOf(name) === -1) name = 'overview';

    navTargets.forEach(function (n) {
      var view = document.getElementById('view-' + n);
      if (view) view.hidden = (n !== name);
    });

    tabs.forEach(function (t) {
      var selected = t.dataset.nav === name;
    ...
    if (name !== 'overview') {
      ...
      if (panel && opts.focus) panel.focus();
    }

    if (opts.pushHash !== false) {
      var hash = name === 'overview' ? ' ' : '#' + name;
      history.replaceState(null, '', name === 'overview' ? location.pathname + location.search : hash);
    }
  ...
  document.querySelectorAll('[data-nav]').forEach(function (el) {
    el.addEventListener('click', function () {
      activate(el.dataset.nav, { focus: true });
    });
  });
  ...
  tablist.addEventListener('keydown', function (e) {
    ...
    next.focus();
    activate(next.dataset.nav);
    e.preventDefault();
  });

  window.addEventListener('hashchange', function () {
    var target = (location.hash || '').replace('#', '');
    activate(navTargets.indexOf(target) !== -1 ? target : 'overview', { pushHash: false });
  });
...
class McpServer {
  ...
  async handleRequest(request, env) {
    ...
    // Live ingest from sync_hands.py — stores the raw hand in R2 and indexes
    // it into KV immediately, so it's searchable without waiting for a
    // backfill pass.
    if (url.pathname === '/hands' && request.method === 'POST') {
      ...
      try {
        ...
        // Non-fatal — the object is safely in R2 either way; a future
        // backfill pass will pick it up if live indexing fails.
      }
...
server.registerTool('get_winrate_by_position', {
  ...
  const sql = `
    ...
    FROM hands
    WHERE hero_position IS NOT NULL AND hero_position != '' AND hero_position != '?'
